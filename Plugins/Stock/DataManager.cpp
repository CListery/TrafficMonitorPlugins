...
const std::string CDataManager::ReadResource()
{
    AFX_MANAGE_STATE(AfxGetStaticModuleState());
    ...
    //HINSTANCE hInst = AfxFindResourceHandle(MAKEINTRESOURCE(IDR_HTML1), _T("HTML"));
    HINSTANCE hInst = AfxFindResourceHandle(MAKEINTRESOURCE(IDR_HTML1), RT_HTML);
    HRSRC hRes = ::FindResource(hInst, MAKEINTRESOURCE(IDR_HTML1), RT_HTML);
    if (!hRes) {
    ...
    std::string result = std::string(static_cast<LPCSTR>(pData), dwSize);
